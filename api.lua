-- Api
foodchain = {}
foodchain.mod = "foodchain"

-- Load settings
local damage_enabled = minetest.setting_getbool("enable_damage")
local peaceful_only = minetest.setting_getbool("only_peaceful_mobs")
local disable_blood = minetest.setting_getbool("mobs_disable_blood")
foodchain.protected = tonumber(minetest.setting_get("mobs_spawn_protected")) or 1

foodchain.remove = false

function foodchain:register_mob(name, def)
...
foodchain.spawning_mobs = {}

function foodchain:spawn_specific(name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height)
	foodchain.spawning_mobs[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not foodchain.spawning_mobs[name] then
			--or not pos then
				return
			end
...
function foodchain:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height)
	foodchain:spawn_specific(name, nodes, {"air"}, min_light, max_light, 60, chance, active_object_count, -31000, max_height)
end
...
-- explosion
function foodchain:explosion(pos, radius, fire, smoke, sound)
...
function foodchain:register_arrow(name, def)
...
-- Spawn Egg
function foodchain:register_egg(mob, desc, background, addegg)
...
function foodchain:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and not self.child then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			if tool:is_empty() then
				chance = chance_hand
			elseif tool:get_name() == "foodchain:net" then
				chance = chance_net
				tool:add_wear(4000) -- 17 uses
				clicker:set_wielded_item(tool)
			elseif tool:get_name() == "foodchain:magic_lasso" then
				-- pick up if owner
				chance = chance_lasso
...
function foodchain:feed_tame(self, clicker, feed_count, breed)
