...
for _, col in ipairs(all_colours) do
	...
	foodchain:register_mob("foodchain:sheep_"..col, {
		type = "animal",
		passive = true,
		hp_min = 8,
		hp_max = 10,
		...
		on_rightclick = function(self, clicker)
			...
			--are we feeding?
			if foodchain:feed_tame(self, clicker, 8, true) then
				--if full grow fuzz
			...
			if itemname == "foodchain:shears" then
				...
				if self.gotten == false and self.child == false and self.tamed == true and name == self.owner then
					...
					for _,c in pairs(all_colours) do
						if c == col then
							local pos = self.object:getpos()
							self.object:remove()
							local mob = minetest.add_entity(pos, "foodchain:sheep_"..col)
	...
	foodchain:register_egg("foodchain:sheep_"..col, "Sheep ("..col..")", "wool_"..col..".png", 1)

end

foodchain:register_spawn("foodchain:sheep_white", {"default:dirt_with_grass", "ethereal:green_dirt"}, 20, 10, 15000, 1, 31000)

-- compatibility (item and entity)
minetest.register_alias("foodchain:sheep", "foodchain:sheep_white")

minetest.register_entity("foodchain:sheep", {
	hp_max = 1,
	physical = true,
	...
	on_rightclick = function(self, clicker)
		clicker:get_inventory():add_item("main", "foodchain:sheep_white")
		self.object:remove()
	end,
